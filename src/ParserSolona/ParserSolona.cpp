...
extern "C"
{
	...
	{
		ParserSolona* parser = new ParserSolona();
		return parser;
	}
...
bool ParserSolona::init(WTSVariant* config)
{
...
void ParserSolona::release()
{
	disconnect();
}

bool ParserSolona::connect()
{
	if(m_pUserAPI)
	{
		m_pUserAPI->Init();
...
void ParserSolona::OnRspError( CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
	IsErrorRspInfo(pRspInfo);
}

void ParserSolona::OnFrontConnected()
{
	if(m_sink)
	{
		write_log(m_sink, LL_INFO, "[ParserSolona] Market data server connected");
...
void ParserSolona::OnRspUserLogin( CThostFtdcRspUserLoginField *pRspUserLogin, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
	if(bIsLast && !IsErrorRspInfo(pRspInfo))
	{
		...
		write_log(m_sink, LL_INFO, "[ParserSolona] Market data server logined, {}", m_uTradingDate);
...
void ParserSolona::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast)
{
	if(m_sink)
	{
...
void ParserSolona::OnFrontDisconnected( int nReason )
{
	if(m_sink)
	{
		write_log(m_sink, LL_ERROR, "[ParserSolona] Market data server disconnected: {}", nReason);
		m_sink->handleEvent(WPE_Close, 0);
	}
}

void ParserSolona::OnRspUnSubMarketData( CThostFtdcSpecificInstrumentField *pSpecificInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{

}

void ParserSolona::OnRtnDepthMarketData( CThostFtdcDepthMarketDataField *pDepthMarketData )
{	
...
void ParserSolona::OnRspSubMarketData( CThostFtdcSpecificInstrumentField *pSpecificInstrument, CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast )
{
...
void ParserSolona::OnHeartBeatWarning( int nTimeLapse )
{
	if(m_sink)
		write_log(m_sink, LL_INFO, "[ParserSolona] Heartbeating, elapse: {}", nTimeLapse);
}

void ParserSolona::ReqUserLogin()
{
	if(m_pUserAPI == NULL)
	{
		return;
	...
	{
		if(m_sink)
			write_log(m_sink, LL_ERROR, "[ParserSolona] Sending login request failed: {}", iResult);
	}
}

void ParserSolona::DoSubscribeMD()
{
	...
	if(m_pUserAPI && nCount > 0)
	{
		...
		{
			if(m_sink)
				write_log(m_sink, LL_ERROR, "[ParserSolona] Sending md subscribe request failed: {}", iResult);
		}
		else
		{
			if(m_sink)
				write_log(m_sink, LL_INFO, "[ParserSolona] Market data of {} contracts subscribed totally", nCount);
		}
	}
	codeFilter.clear();
	delete[] subscribe;
...
bool ParserSolona::IsErrorRspInfo(CThostFtdcRspInfoField *pRspInfo)
{
	return false;
}

void ParserSolona::subscribe(const CodeSet &vecSymbols)
{
	if(m_uTradingDate == 0)
	{
...
void ParserSolona::unsubscribe(const CodeSet &vecSymbols)
{
}

bool ParserSolona::isConnected()
{
	return m_pUserAPI!=NULL;
}

void ParserSolona::registerSpi(IParserSpi* listener)
{
	m_sink = listener;
